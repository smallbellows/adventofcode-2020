const countValid = (input) => {
  ...
  const buildPolicyObj = (policyString) => {
    const [counts, target] = policyString.split(' ')
    const [firstPos, secondPos] = counts.split('-')
    return {
  ...
  const isValid = (password, policyObj) => {
    const {target, firstPos, secondPos} = policyObj
    const passwordArr = password.split('')

    let countMatches = 0
    if (passwordArr[firstPos -1] === target) {
      countMatches ++
    }
    if (passwordArr[secondPos - 1] === target) {
      countMatches ++
    }
    return countMatches === 1
  }
  
  return input.reduce((count, entry) => {
    const [policyString, password] = entry.split(': ')
    const policy = buildPolicyObj(policyString)
...
const testInput = `4-8 n: dnjjrtclnzdnghnbnn
5-6 r: rrrrcqr
2-4 p: pqhpppwpfmr
...
console.log(countValid(testInput.split(/\r?\n/)))
